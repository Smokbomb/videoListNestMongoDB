...
export class VideoService {
    ...
    async updateLike(id) {
        ...
        await this.videoList.forEach(data => {
            if (data.id === id) {
                data.like = Number(data.like) + 1;
                data.score = Number(data.score) + 5;
            }
        });

    }
    async updateView(id) {
        this.videoList.forEach(data => {
            if (data.id === id) {
    ...
    async findById(id) {
        return await this.videoList.filter(data => data.id === id);
    }
}
